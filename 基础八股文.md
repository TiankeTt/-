...
pnpm 速度更快，更加节省空间。比如你使用 npm，10 个项目都使用 React，就会有 10 份 React。pnpm 的话 10 个项目就只有 1 份儿

更安全的隔离

## AI 篇

### 大模型、AI Agent、Vibe Coding、Claude Code & Trae
...
1. 大模型：底层智能引擎
   ...
   - 一切的大脑
   - 是什么：GPT、Claude、文心一言这些通过海量数据训练出来的大型神经网络。它们擅长理解、生成、
     推理，但天生是“静态”的 --- 你问它答，它无法主动采取行动。

   - 作用：为所有上层应用提供最核心的只能。没有大模型，后面的一切都是空谈。

2. AI Agent：上层智能封装

   - 给大脑装上“手脚”
   - 基于大模型构建的智能体，它不仅能思考，还能调用工具、规划步骤、执行动作。比如大模型+读取文件的能力+执行命令的能力 = 一个能帮你写代码的 AI Agent
   - 作用：把大模型从“只会说”升级为“能干活的”。Claude Code、Trae 里的智能体，本质上都是 AI Agent 在编程场景里的具体化身。

3. Vibe Coding：人机协作方法论
   - 一种新的编程“玩法”
   - 是什么：AI Agent 足够强大时，开发者就可以用一种全新的方式来编程——用自然语言描述想法，让 AI 去实现，自己则负责审核和把关。这种“跟着感觉走”的开发方式就叫 Vibe Coding。
   - 作用：定义了人和 AI 如何协作的新范式。开发者从“手写代码”变成“引导 AI 生产代码”。
4. Claude Code & Trae：落地工具（两种不同流派）

如果面试官问：“你如何看待大模型、AI Agent、Vibe Coding 以及 Claude Code、Trae 这些概念的关系？”

“我觉得这是一个层层递进的关系。首先，大模型是所有智能的根基，好比人的大脑。
然后，基于大模型，我们造出了 AI Agent，这相当于给大脑装上了手和脚，让它能自己去执行任务、调用工具。
接着，当 AI Agent 足够能干时，就催生出一种新的编程范式——Vibe Coding，开发者不再逐行写代码，而是像指挥一个聪明的实习生一样，用自然语言描述需求，让 AI 去实现。
最后，这种范式落地到具体工具上，就出现了两种典型代表：一个是 Claude Code，它作为一个终端智能体，特别灵活，能融入我现有的任何工作流；另一个是 Trae，它作为一个 AI 原生 IDE，提供了一站式的全流程自动化体验。它们本质上都是 AI Agent 在编程领域的具体实践，但各有侧重，也相互补充。”
